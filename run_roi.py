...
csv_file2 = project_root + '/sa_all_2/rectangle.csv'
csv_file3 = project_root + '/sa_all_4/rectangle.csv'
save_ae_weights_folder = project_root + '/ae_w.pth'
save_model_weights_folder = project_root + '/model_w.pth'
save_model_weights_finished_folder = project_root + '/model_w_finished.pth'
...
loader_valid = torch.utils.data.DataLoader(ds_valid,batch_size=64, num_workers=0)
...
roi.learn_ae(loader_train, optimizer = opt, criterion = crit,  ep = 500, dataset_validation = loader_valid, 
...
ds_valid = RoiDataset(csv_file2, compose1, compose2,smpl = 256)
ds_test = RoiDataset(csv_file3, compose1, compose2,smpl = 256)
torch.cuda.empty_cache()

loader_train = torch.utils.data.DataLoader(ds_train,batch_size=512, shuffle=True,num_workers=0)
loader_valid = torch.utils.data.DataLoader(ds_valid,batch_size=128, shuffle=False,num_workers=0)
loader_test = torch.utils.data.DataLoader(ds_test, batch_size=128, shuffle=False,num_workers=0)
torch.cuda.empty_cache()
...
roi.learn_roi(loader_train, optimizer, criterion, ep = 1000, dataset_validation = loader_valid,
              ...
              plot_filename = 'plot_first_train.png', plot_test_filename = 'plot_test_first_train.png')
...
roi.learn_roi(loader_train, optimizer, criterion, ep = 1000, dataset_validation = loader_valid, 
              ...
              plot_filename = 'plot_second_train.png', plot_test_filename = 'plot_test_second_train.png')
print('Save weights')

roi.save_model_weights(save_model_weights_finished_folder)
