...
ds_valid = AEDataset(csv_file2, compose3, sample_size = 512)

loader_train = torch.utils.data.DataLoader(ds_train,batch_size=512, num_workers=0)
loader_valid = torch.utils.data.DataLoader(ds_valid,batch_size=512, num_workers=0)
...
ds_valid = RoiDataset(csv_file2, compose1, compose2,smpl = 512)
ds_test = RoiDataset(csv_file3, compose1, compose2,smpl = 512)
torch.cuda.empty_cache()

loader_train = torch.utils.data.DataLoader(ds_train,batch_size=512, shuffle=True,num_workers=0)
loader_valid = torch.utils.data.DataLoader(ds_valid,batch_size=512, shuffle=False,num_workers=0)
loader_test = torch.utils.data.DataLoader(ds_test, batch_size=512, shuffle=False,num_workers=0)
torch.cuda.empty_cache()
